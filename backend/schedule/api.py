...
class EventViewset(PrefetchQuerysetModelMixin, viewsets.ModelViewSet):
    ...
    permission_classes = [Or(And(CanAttendEvents, IsReadyOnlyRequest), IsAdmin)]
...
class RegisteredEventViewset(
    ...
    def get_queryset(self):
        ...
        self.queryset = self.queryset.filter(attendees__in=[attendee]).exclude(
            attended__in=[attendee]
        )
        return super().get_queryset()
    ...
    def post(self, request):
        ...
        if not removed:
            return response.Response(
                {"message": "Participante não se registrou para esse evento"},
                status=403,
            )
        return response.Response({"message": "Registrado"})
...
class FetchCheckinAttendee(views.APIView):
    ...
    def post(self, request):
        ...
        if not event.exists():
            return response.Response(
                {"status": "error", "title": "Opa!", "message": "Evento inválido"}
            )
        attendee = Profile.objects.filter(
            Q(unique_id=unique_id) | Q(user__email=unique_id)
        )
        if not attendee.exists():
            return response.Response(
                {"status": "error", "title": "Opa!", "message": "Participante inválido"}
            )
        event = event[0]
        attendee = attendee[0]
            ...
            return response.Response(
                {
                    "status": "error",
                    "title": "Opa!",
                    "message": "Participante já fez check-in nesse evento",
                }
            )
        return response.Response(
            {
                "status": "success",
                "title": attendee.shortcuts.full_name,
                "message": f"Fazer check-in em {event.name}?",
            }
        )
...
class CheckinAttendee(views.APIView):
    ...
    def post(self, request):
        ...
        return response.Response(
            {"status": "success", "title": "Pronto!", "message": "Check-in completo"}
        )
