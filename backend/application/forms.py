            ...
            if content_type in self.content_types:
                if file.size > self.max_upload_size:
                    raise forms.ValidationError(
                        f"O tamanho máximo permitido é {filesizeformat(self.max_upload_size)}. Tamanho atual: {filesizeformat(file.size)}"
                    )
            else:
...
class ApplicationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "referrer": forms.fields.TextInput(
                attrs={"placeholder": "Amigo, Facebook, carta..."}
            ),
            "interests": forms.fields.TextInput(
                attrs={"placeholder": "Consultoria, financeiro, pesquisa..."}
            ),
            "time_slots": forms.fields.TextInput(
                attrs={"placeholder": "Seg a sexta, 9 às 17..."}
            ),
            "extra_courses": forms.fields.TextInput(
                attrs={"placeholder": "Cozinhando 101, Coursera..."}
            ),
            "other_languages": forms.fields.TextInput(
                attrs={"placeholder": "Klingon avançado"}
            ),
        }
