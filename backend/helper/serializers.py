...
class TicketSerializer(PrefetchMixin, serializers.ModelSerializer):

    creator = SimpleProfileSerializer(read_only=True)
    claimer = SimpleProfileSerializer(read_only=True)
    claimer_unique_id = serializers.CharField(
...
class TicketSubscriptionSerializer(SingleInstancePrefetchMixin, TicketSerializer):
    """Allow single instance prefetching"""


class MentorSerializer(PrefetchMixin, serializers.ModelSerializer):
...
class MentorSubscriptionSerializer(SingleInstancePrefetchMixin, MentorSerializer):
    """Allow single instance prefetching"""
