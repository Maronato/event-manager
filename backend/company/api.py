...
class CompanyViewset(PrefetchQuerysetModelMixin, viewsets.ModelViewSet):
...
class ScanViewset(
    ...
    def get_queryset(self):
        self.queryset = self.queryset.filter(
            scanner__employee__company=self.request.user.profile.employee.company
        )
...
class FetchScanHacker(views.APIView):
    ...
    def post(self, request):
        ...
        if profile.scanned_me.filter(
            scanner__employee__company__id=scanner.employee.company.id
        ).exists():
            return views.Response(
                {
...
class ScanHacker(views.APIView):
    ...
    def post(self, request):
        ...
        scan.save()
        return views.Response(
            {"message": "Scan complete", "id": scan.id, "name": profile.full_name}
        )
