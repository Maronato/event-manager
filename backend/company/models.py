...
class Employee(models.Model):
    msocks_allow = True
    msocks_serializer = "company.serializers.EmployeeSubscriptionSerializer"
...
class Scan(models.Model):
    ...
    scannee = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="scanned_me"
    )
    # Person who scanned
    scanner = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="i_scanned"
    )
    ...
    def scan_person(scannee, scanner):
        scan = Scan(profile=scannee, scanner=scanner)
        scan.save()
